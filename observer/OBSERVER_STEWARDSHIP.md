...
| Composition recalculation (tag weights) | 1 | Reversible, one cluster |
| Stakeholder reinference | 2 | May affect tone for multiple recipients |
| Feature spawn (new tool/feature) | 2 | Cluster-wide effect, reversible |
| Tone adjustment per recipient | 1 | Reversible, one cluster |
| UI layout change | 2 | Visible to all members |
| Cluster spawn proposal | 3 | Structural — creates new cluster |
| Reversal of prior evolution | 1 | Reverses own previous action |
...
### `cluster_prompt_audit` *(NEW — cluster-admin-facing prompt history)*

Denormalized from `observer_prompt_updates`. Human-readable summaries only — no raw prompt text (protocol disclosure protection).

```sql
CREATE TABLE cluster_prompt_audit (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  cluster_id UUID NOT NULL REFERENCES clusters(id),
  update_id UUID REFERENCES observer_prompt_updates(id),

  -- Human-readable summary (generated by Observer)
  summary TEXT NOT NULL,

  -- What changed
  agent VARCHAR(32) NOT NULL,  -- 'sage' | 'clio'
  aspect VARCHAR(64) NOT NULL,  -- 'register' | 'formality' | 'interjection_frequency' | 'citation_mode'
  change_direction VARCHAR(32) NOT NULL CHECK (change_direction IN ('increased', 'decreased', 'shifted', 'added', 'removed')),

  -- Why it changed
  trigger_source VARCHAR(64) NOT NULL,  -- 'genesis_engine' | 'cim_behavioural' | 'cim_functional' | 'admin_request' | 'observer_auto'
  trigger_description TEXT,

  -- Impact tracking (updated 7 days post-commit)
  impact_measured_at TIMESTAMPTZ,
  impact_summary TEXT,
  impact_delta_pct NUMERIC(5,2),
  impact_assessment VARCHAR(32) CHECK (impact_assessment IN ('positive', 'neutral', 'negative', 'pending')),

  -- Admin controls
  status VARCHAR(32) NOT NULL DEFAULT 'auto_applied'
    CHECK (status IN ('auto_applied', 'admin_vetoed', 'admin_approved', 'rolled_back')),
  admin_notified_at TIMESTAMPTZ,
  admin_vetoed_at TIMESTAMPTZ,
  admin_veto_reason TEXT,

  created_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE INDEX idx_cluster_prompt_audit_cluster
  ON cluster_prompt_audit(cluster_id, created_at DESC);
```

**Cluster admin permissions:**
- View every prompt change with human-readable summary
- One-click rollback to previous prompt state (within 30 days)
- Request manual review: "Sage feels off" button → triggers Observer introspection with `prompt_quality_decline = 25`
- See 7-day impact assessment
- Veto upcoming changes during Tier 1/2 veto window

**Cluster admin CANNOT:**
- See raw prompt text or system instructions
- Edit prompts directly
- See other clusters' histories
...
### Reversal Mechanism (Evolution Governor integration)

When the Evolution Governor detects that a committed change worsened member experience, it can propose a reversal. Reversals follow the same tier structure as original proposals but are fast-tracked.

**Trigger conditions (any one):**
- Outcome monitoring shows `outcome_verdict: 'worsened'` after 7 days
- `reversal_rate` > 0.30 (30% of members revert behavior)
- Member feedback explicitly requests undo: "Go back to the old way"
- Admin override

**Reversal process:**
1. Observer proposes reversal with `proposal_type: 'reversal'`
2. Cost = same as original change. Tier = same as original or one lower.
3. If original was Tier 1/2 and confidence > 0.80: auto-approve and execute
4. If original was Tier 3: requires admin approval
5. Agent communicates reversal transparently:
   > "I added audio walkthroughs last week, but I've noticed fewer children completing lessons now. Maybe audio isn't the right fix. I'm trying a simpler bar model instead."
6. Original change is archived in `evolution_proposals` with `status: 'reversed'`
7. Reversal outcome is monitored for another 7 days

**Suppression list:**
If a change type is reversed twice for the same cluster within 30 days, that change type is added to the cluster's `suppressed_actions` list for 60 days. Observer will not propose that change type again during suppression.
