...
|-----|---------|----------|-----------------|--------------|---------------|-------------|
| **Just between us** | ✓ default on first open | `/api/clio/ephemeral` | None | None | **Non-PII** | sessionStorage 12h, no server retention |
| **Ask me anything** (AMA) | Opt-in tab switch | `/api/clio/chat` | Last 10 posts + Sage role | Read-only (titles only) | **PII** | localStorage now, server-persistent post-MVP |

Both tabs are private — neither posts to Timeline or shares with Sage. Tab choice is visible only to the user. Per-tab message threads are stored separately by storage class (sessionStorage for non-PII, localStorage for PII). A first-time tooltip explains the privacy difference; once dismissed (`localStorage.aggilo:clio_tabs_tip_dismissed`), it does not return.
...
**Cluster presence (V3.1 addition):** A single Supabase Realtime presence channel per cluster (`cluster:{cluster_id}:presence`) is owned by `ClusterShell` via a React context (`PresenceProvider` at `mvp/src/lib/presence-context.tsx`). Each member's browser tracks `{user_id, nickname, online_at}`; the count and the set of online userIds update without a refresh as members arrive and leave. Two consumers read from the context: `ClusterPresence` (header — total + live count), and `PostCard` (small green dot next to a member's nickname when they're online). Centralising the channel prevents duplicate subscriptions (which would double-count the local user). Single-tenant in MVP — every signed-up profile counts as a member of the active cluster; for multi-cluster post-MVP, a `cluster_members` join is needed.
...
The chatbox panel placement adapts to cluster maturity. This was refined during the V3 Phase 6 MVP UX review:
...
Implementation reference: `mvp/src/lib/sage-prompt.ts` exports `shallowSimilarity()` and `isSagePostRepetitive()`.
