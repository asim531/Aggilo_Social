...
async function test() {
  ...
  try {
    ...
    const ctaUrl = `https://mvp.aggilo.in/c/research-circle-mj?founder=${founder}&email=${encodeURIComponent(tasEmail)}&gender=${gender}&birth_year=${birthYear}&country=${country}`;
    html = html.replace(/href="https:\/\/mvp\.aggilo\.in\/c\/research-circle-mj\/cluster\?[^"]*"/, `href="${ctaUrl}"`);
