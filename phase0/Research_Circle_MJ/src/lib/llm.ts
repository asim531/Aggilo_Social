 ...
 * Falls back to primary provider if the embeddings endpoint fails.
 */
export async function llmEmbedding(text: string): Promise<number[]> {
  const baseUrl = PRIMARY.baseUrl;
  const apiKey = PRIMARY.apiKey;
  const model = PRIMARY.model;
  if (!baseUrl || !apiKey || !model) {
    throw new Error("LLM not configured for embeddings");
  }
  ...
  if (!response.ok) {
    const text = await response.text().catch(() => "");
    throw new Error(`Embedding HTTP ${response.status}: ${text.slice(0, 200)}`);
  }
