...
/**
 * Auth middleware — Research Circle MJ.
 *
 * Two redirects:
 *   - Unauthenticated users hitting /cluster or /admin → /
 *   - Authenticated users hitting / → /cluster
 *
 * basePath note: when the app is mounted under /c/research-circle-mj
...
export async function middleware(request: NextRequest) {
  ...
  // In prod, basePath is "/c/research-circle-mj" and pathname is the same
  ...
  if (!user && (path.startsWith("/cluster") || path.startsWith("/admin"))) {
    const redirectUrl = resolvePublicUrl(request, "/c/research-circle-mj");
...
export const config = {
  ...
  // `/cluster/foo` (dev) and `/c/research-circle-mj/cluster/foo` (prod).
