...
export default async function AdminIndexPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold text-husl-ink">Admin</h1>
        <p className="text-sm text-husl-muted mt-1">
          Cluster operations for Research Circle MJ.
        </p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <Link
          href="/admin/welfare"
          className="block bg-husl-card border border-stone-200 rounded-lg p-5 hover:border-husl-clio transition-colors"
        >
          <div className="flex items-baseline justify-between mb-2">
            <h2 className="text-base font-semibold text-husl-ink">
              Welfare queue
            </h2>
            <span
              className={`text-sm font-semibold ${
                (unresolvedWelfare ?? 0) > 0 ? "text-rose-600" : "text-husl-muted"
              }`}
            >
