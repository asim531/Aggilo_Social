  ...
  return (
    <div className="space-y-6">
      <div className="flex items-baseline justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-husl-ink">Welfare queue</h1>
          <p className="text-sm text-husl-muted mt-1">
            {showResolved
          ...
          <Link
            ...
            className={`px-2 py-1 rounded ${
              !showResolved
                ...
                : "text-husl-muted hover:text-husl-ink"
            }`}
          >
            Open
          </Link>
        ...
        <ul className="space-y-3">
          {items.map((item) => {
            ...
            return (
              <li
                key={item.id}
                className="bg-husl-card border border-stone-200 rounded-lg p-4 space-y-3"
              >
                <div className="flex items-baseline justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-sm text-husl-muted mt-0.5">
                      {profile ? (
                        <>
                ...
                {item.trigger_content && (
                  <blockquote className="text-sm text-husl-ink leading-relaxed border-l-2 border-rose-300 pl-3 py-1 bg-rose-50/40 rounded-r">
                    {item.trigger_content}
                  </blockquote>
                )}

                {post && (
                  <div className="text-xs text-husl-muted">
                ...
                {item.sage_response && (
                  <div className="text-xs text-husl-muted border-t border-stone-100 pt-2">
                    <span className="font-semibold text-husl-sage">Sage:</span>{" "}
                    {item.sage_response}
                  </div>
                )}
