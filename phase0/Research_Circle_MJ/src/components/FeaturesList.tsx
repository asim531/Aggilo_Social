...
/**
 * Workshop list — Research Circle MJ.
 *
 ...
 *   tools use teal (husl-sage) — agents' work
 *   features use amber (husl-clio) — member-driven
 */
...
const FEATURE_STATUS_CHIP: Record<string, { label: string; tone: string }> = {
  ...
  in_features_tab: {
    ...
    tone: "bg-amber-50 text-husl-clio border border-amber-200",
  },
  members_engaged: {
...
const TOOL_STATUS_CHIP: Record<string, { label: string; tone: string }> = {
  deployable_now: {
    ...
    tone: "bg-teal-50 text-husl-sage border border-teal-200",
  },
  needs_building: {
    ...
    <div className="space-y-8">
      ...
      {tools.length > 0 && (
        <section>
          <header className="mb-3">
            <h3 className="text-sm font-semibold text-husl-ink flex items-center gap-2">
              <span className="text-husl-sage">🛠️</span>
              Tools we run for this room
            </h3>
            <p className="text-xs text-husl-muted mt-0.5">
          ...
          <ul className="space-y-3">
            {tools.map((tool) => {
              ...
              return (
                <li
                  key={tool.id}
                  className="bg-husl-card rounded-xl border border-teal-100 p-4 shadow-sm"
                >
                  <div className="flex items-start justify-between gap-3 mb-2">
                    <div className="flex-1 min-w-0">
                      <h4 className="text-base font-semibold text-husl-ink mb-1">
                        {tool.display_name}
                      </h4>
                      <p className="text-sm text-husl-muted leading-relaxed">
                  ...
                  {tool.rationale && (
                    <p className="text-xs text-husl-muted italic border-l-2 border-teal-200 pl-2 mb-2">
                      {tool.rationale}
                    </p>
                  )}

                  <div className="flex items-center justify-between text-xs text-husl-muted">
                    <span>
      ...
      {memberFeatures.length > 0 && (
        <section>
          <header className="mb-3">
            <h3 className="text-sm font-semibold text-husl-ink flex items-center gap-2">
              <span className="text-husl-clio">✦</span>
          ...
          <ul className="space-y-3">
            {memberFeatures.map((f) => {
              ...
              return (
                <li
                  key={f.id}
                  className="bg-husl-card rounded-xl border border-stone-200 p-4 shadow-sm"
                >
                  <div className="flex items-start justify-between gap-3 mb-2">
                    <div className="flex-1 min-w-0">
                      <h4 className="text-base font-semibold text-husl-ink mb-1">
                  ...
                  {f.rationale && (
                    <p className="text-xs text-husl-muted italic border-l-2 border-amber-200 pl-2 mb-3">
                      {f.rationale}
                    </p>
                  )}
                  ...
                  <div className="flex items-center justify-between text-xs">
                    <button
                      ...
                      className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full border transition-colors ${
                        isUpvoted
                          ? "bg-amber-50 border-amber-300 text-husl-clio"
                          : "border-stone-200 text-husl-muted hover:border-amber-300 hover:text-husl-clio"
      ...
      {tools.length === 0 && memberFeatures.length === 0 && (
        <div className="bg-husl-card border border-dashed border-stone-300 rounded-lg p-8 text-center">
