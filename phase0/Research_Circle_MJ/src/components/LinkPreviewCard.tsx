...
export default function LinkPreviewCard({ url }: LinkPreviewCardProps) {
  ...
  if (loading) {
    return (
      <div className="my-2 px-3 py-2 rounded-lg border border-stone-200 dark:border-stone-700 bg-stone-50 dark:bg-[#1a1d22] animate-pulse">
        <div className="h-3 w-1/3 bg-stone-200 dark:bg-stone-700 rounded mb-2" />
        <div className="h-2 w-2/3 bg-stone-200 dark:bg-stone-700 rounded" />
      </div>
    );
  }
    ...
    return (
      <a
        ...
        className="my-2 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg
                   border border-stone-200 dark:border-stone-700 bg-stone-50 dark:bg-[#1a1d22] text-xs text-husl-muted dark:text-stone-400
                   hover:bg-stone-100 dark:hover:bg-stone-800 transition-colors"
      >
        <svg className="w-3.5 h-3.5 shrink-0 text-stone-400 dark:text-stone-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  return (
    <div className="my-2">
      <a
        ...
        target="_blank"
        rel="noopener noreferrer nofollow"
        className="block rounded-lg border border-stone-200 dark:border-stone-700 bg-husl-card hover:border-stone-300 dark:hover:border-stone-600 hover:shadow-sm transition-all overflow-hidden"
      >
        <div className="flex">
          {preview.image_url && (
            <div className="hidden sm:block w-24 h-24 shrink-0 bg-stone-100 dark:bg-stone-800 overflow-hidden border-r border-stone-100 dark:border-stone-700">
          ...
          <div className="flex-1 min-w-0 p-3 flex flex-col justify-center">
            <div className="flex items-center gap-1.5 text-[10px] text-husl-muted dark:text-stone-400 uppercase tracking-wide mb-1">
              <svg className="w-3 h-3 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" />
              </svg>
              <span className="truncate">{preview.site_name || new URL(preview.url).hostname}</span>
            </div>
            <p className="text-sm font-medium text-husl-ink dark:text-stone-200 line-clamp-2 mb-1 leading-snug">
              {preview.title}
            </p>
            {preview.description && (
              <p className="text-xs text-husl-muted/80 dark:text-stone-400/80 line-clamp-2">
                {preview.description}
              </p>
            )}
          </div>
  ...
  while ((m = URL_RE.exec(text))) {
    ...
    nodes.push(
      <a
        ...
        target="_blank"
        rel="noopener noreferrer nofollow"
        className="text-husl-clio hover:underline break-all"
      >
        {m[1]}
      </a>
    );
