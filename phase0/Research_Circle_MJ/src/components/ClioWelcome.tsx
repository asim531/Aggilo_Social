...
/**
 * ClioWelcome — Research Circle MJ orientation modal.
 *
  ...
  const steps: Step[] = [
    {
      ...
      body: (
        <>
          <p className="mb-2">
            This is <strong>Research Circle MJ</strong>. Nicknames only. Documents, images, and videos stay linked to their topics.
          </p>
          <p className="text-husl-muted text-sm">
            Topics keep everything findable. If a thread shifts, Sage will suggest where it belongs.
          </p>
        </>
      ),
    },
    {
      ...
      body: (
        <>
          <p className="mb-2">
            <span className="font-semibold text-husl-sage">Sage</span> anchors the room. She speaks rarely, only when she has something to add.
          </p>
          <p className="mb-2">
            <span className="font-semibold text-husl-clio">Clio</span> — me — is for you. Tap the button bottom-right when you want me. Private mode lives in the same panel.
          </p>
        </>
      ),
    },
      ...
      <div className="bg-husl-card rounded-2xl shadow-2xl max-w-sm w-full overflow-hidden">
        ...
        <div className={`px-5 pt-3 pb-0 border-b ${current.borderColor}`}>
          <div className="flex items-center justify-between mb-1">
            <span className="text-[10px] uppercase tracking-[0.18em] text-husl-muted font-medium">
              Research Circle MJ
            </span>
            <button
              type="button"
          ...
          <h2
            ...
            className="text-lg font-semibold text-husl-ink pb-3"
          >
            {current.title}
          </h2>
        </div>
            ...
            {steps.map((_, i) => (
              <span
                ...
                className={`h-1.5 rounded-full transition-all duration-300 ${
                  i === step ? "w-3 bg-husl-clio" : "w-1.5 bg-stone-200"
                }`}
              />
            ))}
          </div>
          ...
          <div className="flex items-center gap-1.5">
            {step > 0 && (
              <button
                ...
                className="text-xs text-husl-muted hover:text-husl-ink transition-colors px-2 py-1"
              >
                Back
              </button>
            )}
