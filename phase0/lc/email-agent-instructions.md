...
## Goal
Implement a founder invite email feature for the Long Conversation cluster. Email sends only when explicitly triggered by the command: `send email to founder: [email1, email2, ...]`
...
## 3. File structure to create inside `phase0/lc/`
```
phase0/lc/
├── emails/
...
## 4. Create `phase0/lc/app/api/send-founder-invite/route.ts`
...
export async function POST(req: Request) {
  ...
  const baseHtml = readFileSync(
    join(process.cwd(), 'phase0/lc/emails/clio-invite-email.html'),
    'utf-8'
  );

  const results = await Promise.allSettled(
    recipients.map(({ email, name }: { email: string; name: string }) => {
      const html = baseHtml.replace('Hey Tas,', `Hey ${name},`);
...
## 5. Create the trigger utility `phase0/lc/lib/sendFounderInvite.ts`
...
import { sendFounderInvite } from '@/phase0/lc/lib/sendFounderInvite';

// Example trigger:
await sendFounderInvite([
  { email: 'tas@example.com', name: 'Tas' },
...
```
phase0/lc/emails/clio-invite-email.html
```
