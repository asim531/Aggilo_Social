...
> **Status:** Design document (Phase 0). Specifies two new Clio
> interactions that surface in the AMA (the conversational FAB
> interface): the cluster creation workflow for any member who wants
> to start their own room, and the founding member feedback loop that
> closes the intake pipeline (Pattern 7 in
...
> and voice baseline.
>
> **Implementation status:** Spec only. To be built after the
> Long Conversation cluster shell + Sage worker + Clio FAB are live.
>
> **Created:** 2026-05-25 in response to product review of the
> Long Conversation onboarding flow. Captures the two new interactions
> agreed during that review.
