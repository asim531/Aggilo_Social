...
/**
 * Workshop list — renders both tracks of the two-track capability model:
 *
 *   1. Agent Tools (kind: 'agent_tool')
 *      - Things Sage/Clio run for the room
 *      - Members never click; they receive output
 *      - No upvote UI; instead show invocation count and build status
 *      - "Already running" or "We'll build this"
 *
 *   2. Member Features (kind: 'member_feature')
 *      - UI surfaces members touch
 *      - Vote-gated; show upvote button + comment count
 *      - "Open for feedback" / "In development" / "Now live"
 *
 * Visual register: tools use cyan accents (agents' work), features use
 * amber accents (member-driven). The visual distinction reinforces the
 * mental model — tools are infrastructure, features are interaction.
 */
...
interface FeatureRow {
  ...
  created_at: string;
  // Two-track fields (added v1.7)
  kind?: "agent_tool" | "member_feature" | null;
  build_status?: "deployable_now" | "needs_building" | "building" | "live" | "paused" | "retired" | null;
...
const TOOL_STATUS_CHIP: Record<string, { label: string; tone: string }> = {
  deployable_now: { label: "Already running", tone: "bg-cyan-50 text-cyan-700 border border-cyan-200" },
  needs_building: { label: "We&apos;ll build this", tone: "bg-cyan-50 text-cyan-700 border border-cyan-200" },
  building: { label: "Being built", tone: "bg-sky-50 text-sky-700 border border-sky-200" },
  live: { label: "Live", tone: "bg-emerald-50 text-emerald-700 border border-emerald-200" },
  paused: { label: "Paused", tone: "bg-gray-100 text-gray-500" },
  retired: { label: "Retired", tone: "bg-gray-100 text-gray-400" },
};
...
export default function FeaturesList({ features, tier, userId }: Props) {
  ...
  const memberFeatures = rows.filter((r) => r.kind !== "agent_tool"); // includes nulls (legacy rows)
  ...
  return (
    <div className="space-y-8">
      {/* ── Agent Tools — what we run for the room ────────────────── */}
      {tools.length > 0 && (
        <section>
          <header className="mb-3">
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              These run in the background. Members receive output, no clicks needed.
            </p>
          </header>
          <ul className="space-y-3">
            {tools.map((tool) => {
              ...
              return (
                <li
                  key={tool.id}
                  className="bg-white rounded-xl border border-cyan-100 p-4 shadow-sm"
                >
                  ...
                  {tool.rationale && (
                    <p className="text-xs text-gray-500 italic border-l-2 border-cyan-200 pl-2 mb-2">
                      {tool.rationale}
                    </p>
                  )}
      ...
      {/* ── Member Features — for you to vote on ──────────────────── */}
      {memberFeatures.length > 0 && (
        <section>
          <header className="mb-3">
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              These need member feedback. Your upvote tells the agents and admin what to prioritise.
            </p>
          </header>
          <ul className="space-y-3">
            {memberFeatures.map((f) => {
              const chip =
                FEATURE_STATUS_CHIP[f.status] ?? FEATURE_STATUS_CHIP.in_features_tab;
              ...
              return (
                ...
                >
                  ...
                  {f.rationale && (
                    <p className="text-xs text-gray-500 italic border-l-2 border-amber-200 pl-2 mb-3">
                      {f.rationale}
                    </p>
                  )}
                  ...
                  <div className="flex items-center justify-between text-xs">
                    ...
                    {f.proposed_by === "agents_joint" ? (
                      ...
                      <span className="text-gray-400">Proposed by a member</span>
                    ) : (
                  ...
                  {tier !== "active" && (
                    <p className="text-[11px] text-gray-400 mt-2">
                      Voting and comments open when the room has 15 members. Your presence shapes what gets built.
                    </p>
                  )}
                </li>
              );
