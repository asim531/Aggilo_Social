...
import { DEFAULT_CLUSTER_ID } from "@/lib/prompts/registry";
...
export default function ClioTipLayer({ userId }: { userId: string }) {
  ...
  useEffect(() => {
    ...
    async function checkTips() {
      const { data } = await supabase
        .from("clio_tip_log")
        .select("*")
        .eq("cluster_id", DEFAULT_CLUSTER_ID)
        .eq("user_id", userId)
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload: any) => {
          if (payload.new && payload.new.cluster_id === DEFAULT_CLUSTER_ID) {
  ...
  return (
    <div className="fixed bottom-24 right-4 z-[90] max-w-sm animate-in slide-in-from-bottom-8 fade-in duration-500">
      <div className="relative bg-white border border-stone-200 shadow-xl rounded-xl p-4 pl-12 overflow-hidden">
        {/* Accent left border — amber, matching Clio's signature colour */}
        <div className="absolute left-0 top-0 bottom-0 w-1.5 bg-aggilo-accent" />
