...
export default function LinkPreviewCard({ url }: LinkPreviewCardProps) {
  ...
  if (loading) {
    return (
      <div className="my-2 px-3 py-2 rounded-lg border border-gray-200 bg-gray-50 animate-pulse">
    ...
    return (
      <a
        ...
        className="my-2 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg
                   border border-gray-200 bg-gray-50 text-xs text-gray-600
  ...
  return (
    <div className="my-2">
      <a
        ...
        target="_blank"
        rel="noopener noreferrer nofollow"
        className="block rounded-lg border border-gray-200 bg-white hover:border-aggilo-deep/30 hover:shadow-sm transition-all overflow-hidden"
      >
        <div className="flex">
          {preview.image_url && (
            <div className="hidden sm:block w-24 h-24 shrink-0 bg-gray-100 overflow-hidden">
              {/* Plain img for arbitrary external URLs — next/image needs config per-domain */}
          ...
          <div className="flex-1 min-w-0 p-3">
            <div className="flex items-center gap-1.5 text-[10px] text-gray-400 uppercase tracking-wide mb-1">
      ...
      {/* Sage's on-topic badge — only when confident.
          Off-topic and unsure verdicts are silent. The badge is the only
          surface; silence is the right answer for everything else. */}
      {preview.sage_verdict === "on_topic" && (
        <div className="mt-1.5 inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-50 border border-emerald-200 text-[11px] text-emerald-700">
          <svg className="w-3 h-3 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" />
          </svg>
          <span>On topic — Sage</span>
        </div>
      )}

      {/* off_topic and unsure: no badge, no callout. Sage stays quiet. */}
    </div>
  );
}
  ...
  while ((m = URL_RE.exec(text))) {
    ...
    nodes.push(
      <a
        ...
        target="_blank"
        rel="noopener noreferrer nofollow"
        className="text-aggilo-accent hover:underline break-all"
      >
        {m[1]}
      </a>
    );
